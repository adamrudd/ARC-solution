...
if RUN_DEPTH_23: # Add XX this line and indent contents
    flip3 = []
    for i in range(ntasks):
        print(f"Processing task {i+1} of {ntasks}") # XX
        status, t, m = stats3[depth3[i].cmd]
        flip3.append(Command("./run %d 23"%i, t*2, m*2, 100))
    stats3_flip = runAll(flip3, 4)

if RUN_DEPTH_33: # Add XX this line and indent contents
    flip3 = []
    for i in range(ntasks):
        status, t, m = stats3[depth3[i].cmd]
...
if RUN_DEPTH_4: # Add XX this line and indent contents
    depth4 = []
    for i in range(ntasks):
        status, t, m = stats3[depth3[i].cmd]
